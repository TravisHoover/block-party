...
| File | Slot | Pixels |
| --- | --- | --- |
| `iphone-6.5-1-gameplay.png` | iPhone 6.5" Display | 1242 × 2688 |
| `iphone-6.5-2-clear.png` | iPhone 6.5" Display | 1242 × 2688 |
| `iphone-6.5-3-newbest.png` | iPhone 6.5" Display | 1242 × 2688 |
| `iphone-6.9-1-gameplay.png` | iPhone 6.9" Display | 1320 × 2868 |
| `iphone-6.9-2-clear.png` | iPhone 6.9" Display | 1320 × 2868 |
...
**Which iPhone slot App Store Connect offers depends on the Xcode SDK the
build was uploaded with — upload whichever set it asks for.** Build 5 was
uploaded with an older SDK, so its listing offers only a **6.5"** slot and
rejects 1320 × 2868 outright ("The dimensions of one or more screenshots are
wrong"). Keep both sets: 6.5" for build 5, 6.9" for whenever a build goes up
from a current SDK. Accepted 6.5" sizes are 1242 × 2688 or 1284 × 2778.

Upload order = display order. Smaller devices reuse these automatically.
The **app icon** uploads with the build itself — nothing to provide
separately.

**These are real captures of build 5, taken in the iOS Simulator** — iPhone 11
Pro Max (6.5"), iPhone 17 Pro Max (6.9") and iPad Pro 13-inch, all running the
code at commit `69267a5`. Verify any replacement the same way: capture from the
build you are actually shipping, never from `main` and never from a mock.

No 6.5" simulator ships by default any more, but the device type still exists —
create one with
`xcrun simctl create shot-65 com.apple.CoreSimulator.SimDeviceType.iPhone-11-Pro-Max com.apple.CoreSimulator.SimRuntime.iOS-18-3`
and build against that destination.

The previous set was generated by an HTML mock and **deliberately showed
color themes and the confetti celebration**. Those are PR #8 features that
do not exist in build 5, which made the live listing inaccurate under
Guideline 2.3.3 — a real risk on a Made for Kids app, where metadata gets
closer scrutiny. Note also that the mock's colors never matched the app:
the real Classic theme is a much brighter violet than the mock's magenta
and near-black navy.

The old 6.5" set had the same confetti problem and has been replaced with real
6.5" captures rather than dropped — build 5's listing accepts nothing else.

When 1.1 ships with the polish pack, themed and confetti screenshots
become accurate and should be recaptured then — from the 1.1 binary.
