...
**These are real captures of build 5, taken in the iOS Simulator** —
iPhone 17 Pro Max and iPad Pro 13-inch, running the code at commit
`69267a5`. Verify any replacement the same way: capture from the build
you are actually shipping, never from `main` and never from a mock.
...
**The 6.5" set was deleted.** It had the same confetti problem and cannot
be regenerated — no 6.5" simulator exists any more. Apple's current
requirement is a 6.9" iPhone plus a 13" iPad, and it reuses those for
smaller sizes. In App Store Connect, delete the stale 6.5" screenshots
still sitting in that slot and upload the 6.9" set instead.
